...
void CollisionChecker::Update(std::vector<Agent*>* agents, std::vector<Agent*>* currentlyCollidingObjects)
{
	// Check each agent's collision (Collider)
	for (iterators[0] = 0; iterators[0] < agents->size(); ++iterators[0])
	{
		// Setting the collider
		objectsBeingChecked[0] = static_cast<ObjectTransform*>(agents->at(iterators[0])->GetTransformPtr());
		...
		for (iterators[1] = 0; iterators[1] < agents->size(); ++iterators[1])
		{
			...
			if (iterators[1] != iterators[0])
			{
				...
				objectsBeingChecked[1] = static_cast<ObjectTransform*>(agents->at(iterators[1])->GetTransformPtr());
